...
function generateQuestData(name, type, coordinates) {
  ...
  return {
    ...
    solutionWord: Math.random() > 0.5 ? `lösung_${name.toLowerCase().replace(/[^a-z0-9]/g, '_')}` : undefined,
      ...
      if (values.length >= 4) {
        ...
        if (coordinates) {
          if (type === 'QuestArea') {
            ...
            const questData = generateQuestData(name, type, coordinates);
